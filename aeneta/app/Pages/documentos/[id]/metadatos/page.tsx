"use client";
import { LinkVizualizacionDocumento } from "@/app/Components/botones_links/botones";
    ...
    try {
      ...
      const fetchMetadatosDocumento = async () => {
        const response = await fetch(
          "http://localhost:4000/documentos/metadatos",
          {
            method: "POST",
            headers: {
        ...
        const documentosFormateado = data.map(
          (documento: {
            id: number;
            Titulo: string;
    ...
    <main className="">
      ...
      {documento.map(
        (documento: {
          ...
          <div
            className="mx-auto mt-2 w-fit border rounded-2xl flex flex-col p-2"
            key={documento.id}
          >
            <h2 className="font-bold text-3xl text-center mt-2">
              {documento.Titulo.toUpperCase()}
            </h2>
